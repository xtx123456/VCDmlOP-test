...
from pot_core.data import make_owner_and_aux_loaders, num_classes_of
...
def main():
    ...
    # 3) Data split (owner / aux) —— 与你原有的接口保持一致
    owner_loader, owner_val_loader, aux_loader = make_owner_and_aux_loaders(
        dataset,
        root=args.data,
    ...
    meta = {
        ...
        'notes': 'val_acc = student-teacher agreement on held-out AUX slice',
    }
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.verbose:
            print(f"[attack/distill] epoch {epoch}/{args.epochs} "
                  f"| agree={val_agree:.3f} | T={T_now:.2f} | lam={lam_now:.3f}")
