...
def main():
    ap = argparse.ArgumentParser(description='MAMMON verify (no data)')
    ...
    if os.path.isfile(meta_path):
        try:
            ...
            if ds:
                title = f"Verification Results ({ds})"
        except Exception:
            pass
    print("\n" + title)
    print("-" * len(title))
    ...
    # 固定顺序打印关键指标
    for k in ['rho_val_acc','max_EMD_consecutive','max_EMD_init_vs_GMM',
              'max_PCA_ratio_init','rho_neg_weight_distance',
              'init_final_distance','random_mean','random_std',
              'init_final_distance_is_small_vs_random','EPOCHS']:
        print(f"{k}: {res[k]}")
