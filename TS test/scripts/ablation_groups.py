...
# 修改后的分组（P2不参与，P3和P4分到一组）
GROUPS = {
    "G1_P1": ["P1"],
    "G34_P3P4": ["P3", "P4"],  # P3和P4合并为一组
    "G6_P6": ["P6"],
}

# 修改后的模式：ALL / DROP / ONLY
MODES = [
    ("ALL",       ["G1_P1", "G34_P3P4", "G6_P6"]),
    ("DROP_P1",   ["G34_P3P4", "G6_P6"]),
    ("DROP_P34",  ["G1_P1", "G6_P6"]),  # 这里去掉P3和P4的组合
    ("DROP_P6",   ["G1_P1", "G34_P3P4"]),
    ("ONLY_P1",   ["G1_P1"]),
    ("ONLY_P34",  ["G34_P3P4"]),
    ("ONLY_P6",   ["G6_P6"]),
]
...
def _mode_enabled_preds(mode_groups: List[str]) -> List[str]:
    """把启用的 group 展开成谓词列表（P1/P3/P4/P6）。"""
    preds = []
...
def eval_chains(
    ...
    num_rand: int,
    seed: int,
    eps2: float, eps3: float, eps4: float, rho1: float, rho5: float, k6: float,
    ...
    for c in chains:
        ...
        pf = apply_strict(res, eps2=eps2, eps3=eps3, eps4=eps4, rho1=rho1, rho5=rho5, k6=k6)
        meta = _read_meta(c)
        per_chain.append({
            "chain": c,
...
def main():
    ...
    ap.add_argument("--rho1", type=float, default=0.95)
    ...
    clean_items, _ = eval_chains(
    clean_chains, args.emd_bins, args.num_rand, args.seed,
    args.eps2, args.eps3, args.eps4, args.rho1, args.rho5, args.k6,
    )
    attack_items, _ = eval_chains(
        attack_chains, args.emd_bins, args.num_rand, args.seed,
        args.eps2, args.eps3, args.eps4, args.rho1, args.rho5, args.k6,
