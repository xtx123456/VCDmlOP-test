...
def verify_chain(chain_dir: str, emd_bins: int = 200, num_random: int = 20, seed: int = 0) -> Dict[str, float]:
    ...
    return {
        ...
        'P6_random_mean': mean_rand,
        'P6_random_std': std_rand,
        'P6_is_small_vs_random': bool(init_final < (mean_rand - 3.0 * std_rand)),
        'EPOCHS': float(epochs),
    }
...
def apply_strict(
    ...
    eps3=0.03,
    eps4=0.25,
    rho1=0.95,   # 新增：P1 阈值，默认就设成 0.95
    rho5=0.6,
    k6=3.0
):
    return {
        'PASS_P1': res['P1_rho_val_acc'] >= rho1,
